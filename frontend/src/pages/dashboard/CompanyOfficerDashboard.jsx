      ...
      <div className="p-4 lg:p-6 space-y-8">
        ...
        {showCreateForm && (
          <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <Card className="border border-border/50 bg-background/50 backdrop-blur-sm rounded-xl">
              ...
              <CardContent className="pt-6">
                <form onSubmit={handleCreateShipment} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <Input placeholder="Cargo Type" value={formData.cargoType} onChange={(e) => setFormData({ ...formData, cargoType: e.target.value })} required />
                    <Input placeholder="Cargo Weight (kg)" type="number" value={formData.cargoWeight} onChange={(e) => setFormData({ ...formData, cargoWeight: e.target.value })} />
                  </div>
                  <Input placeholder="Cargo Description" value={formData.cargoDescription} onChange={(e) => setFormData({ ...formData, cargoDescription: e.target.value })} />
                  <div className="grid grid-cols-2 gap-4">
                    <Input placeholder="Origin Location ID" value={formData.originId} onChange={(e) => setFormData({ ...formData, originId: e.target.value })} required />
                    <Input placeholder="Destination Location ID" value={formData.destinationId} onChange={(e) => setFormData({ ...formData, destinationId: e.target.value })} required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Input placeholder="Receiver Contact" value={formData.receiverContact} onChange={(e) => setFormData({ ...formData, receiverContact: e.target.value })} required />
                  </div>
                  <Input type="datetime-local" value={formData.estimatedArrival} onChange={(e) => setFormData({ ...formData, estimatedArrival: e.target.value })} required />
...
{loading ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {shipments.map((shipment, idx) => (
                <motion.div key={shipment.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}>
                  <Card className={`border border-border/50 hover:shadow-md transition-all h-full bg-gradient-to-br from-background to-secondary/30 ${selectedShipment?.id === shipment.id ? 'ring-2 ring-primary' : ''}`}>
                    ...
                    <CardContent className="space-y-4 pt-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="w-full bg-border/50 rounded-full h-2 overflow-hidden">
                          <motion.div className="bg-primary h-2" initial={{ width: 0 }} animate={{ width: `${shipment.currentProgress}%` }} />
                        </div>
                      </div>
                      <div className="flex gap-2 pt-3">
                        <Button variant="outline" size="sm" className="flex-1" onClick={() => setSelectedShipment(selectedShipment?.id === shipment.id ? null : shipment)}>
                          <Eye className="w-4 h-4 mr-1" /> Details
                        </Button>
                        <Button variant="outline" size="sm" className="px-3"><Edit2 className="w-4 h-4" /></Button>
                        <Button variant="destructive" size="sm" className="px-3" onClick={() => handleDeleteShipment(shipment.id)}><Trash2 className="w-4 h-4" /></Button>
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
